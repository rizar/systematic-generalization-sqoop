...
def vocab_to_module_text(program_vocab):
    ...
    return (programs_token_to_module_text,
            module_token_to_idx,
            text_token_to_idx)
...
def main(shapes_data, size, programs):
    parts = ['train', 'val', 'test']
    part_prefixes = ['train.' + size, 'val', 'test']

    for part, prefix in zip(parts, part_prefixes):
        image_path = "{}/shapes_dataset/{}.input.npy".format(shapes_data, prefix)
        images = numpy.load(image_path)

        questions_path = "{}/shapes_dataset/{}.query_str.txt".format(shapes_data, prefix)
        ...
        programs_path = "{}/shapes_dataset/{}.query".format(shapes_data, prefix)
        programs = []
        ...
        answers_path = "{}/shapes_dataset/{}.output".format(shapes_data, prefix)
